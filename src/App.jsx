...
const ZONES = [
  ...
  { code:"NE", label:"North East England", fee:3.00 },
];
            ...
            <button key={t.id} onClick={()=>{ setView(t.id);
              ...
              {(t.badge||0)>0&&(
                <span style={{ position:"absolute",top:-4,right:-4,width:17,height:17,
                  borderRadius:9,background:B.orange,color:"#fff",fontSize:9,fontWeight:800,
    ...
    <div style={{ height:"100%",background:B.bg,overflowY:"auto" }}>
      <div style={{ background:"linear-gradient(180deg,#FBF0ED 0%,#F8F8F6 100%)",
        ...
        <div style={{ display:"inline-flex",alignItems:"center",gap:8,background:"#fff",
          border:"2px solid #1A6B3A",borderRadius:12,padding:"6px 14px",marginBottom:12 }}>
          <div style={{ display:"flex",gap:2 }}>
            {[1,2,3,4,5].map(s=><span key={s} style={{ fontSize:14,color:B.green }}>★</span>)}
          </div>
          <div>
          ...
          {[["⏱","45–60 min"],["💳","Card / Transfer"],["✓ Halal","All items"]].map(([ic,tx])=>(
            <div key={tx} style={{ display:"flex",flexDirection:"column",alignItems:"center",
              ...
              <span style={{ fontSize:ic.startsWith("✓")?"12px":"18px",
                color:ic.startsWith("✓")?B.green:"inherit",
                fontWeight:ic.startsWith("✓")?700:"inherit" }}>{ic}</span>
              {tx}
            </div>
          ))}
        </div>
      ...
      <div style={{ margin:"12px 20px 0",padding:"10px 14px",background:B.amberLight,
        border:`1px solid ${B.amber}30`,borderRadius:10,fontSize:12,color:B.amber,lineHeight:1.6 }}>
              ...
              <div style={{ display:"flex",alignItems:"flex-start",gap:12,flex:1 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:"flex",gap:8,marginTop:6,alignItems:"center",flexWrap:"wrap" }}>
                    ...
                    <span style={{ fontSize:11,color:B.textMid }}>{m.calories} kcal</span>
                  </div>
                  <div style={{ marginTop:6,display:"flex",gap:6,alignItems:"center",flexWrap:"wrap" }}>
                    <DietBadge halal={m.is_halal} vegan={m.is_vegan} />
                    <button onClick={()=>setShowAllergens(showAllergens===m.id?null:m.id)}
                      style={{ fontSize:10,padding:"2px 8px",borderRadius:6,cursor:"pointer",
                        background:B.amberLight,color:B.amber,border:`1px solid ${B.amber}30`,fontWeight:600 }}>
                      Allergens ⓘ
                    </button>
                  </div>
                  {showAllergens===m.id&&(
      ...
      <div style={{ flex:1,display:"flex",minHeight:0,overflow:"hidden" }}>
            ...
            <div key={o.id} onClick={()=>setSel(sel?.id===o.id?null:o)}
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4 }}>
                <div>
                  ...
                  <div style={{ fontSize:11,color:B.textDim }}>{o.id} · {o.postcode}</div>
                </div>
                <Pill s={o.status} />
              </div>
        ...
        {sel&&(
          <div style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
            ...
            {NEXT[sel.status]&&(
              <Btn full onClick={()=>advance(sel)} style={{ marginBottom:8 }}>
                Mark as {NEXT[sel.status]} + notify 💬
              </Btn>
            )}
            <Btn full v="wa" onClick={()=>openWA(sel.phone,
              `Hello ${sel.customer.split(" ")[0]}, update on your order ${sel.id}: ${sel.status}.`)}>
              💬 Custom message
            </Btn>
          </div>
        )}
...
function RiderApp() {
  ...
  const claim = async (o) => {
    ...
    fetchOrders();
    openWA("447800000000",
      `Hi, I'm claiming order ${o.id} for ${o.customer} (${o.postcode}). On my way to collect now 🛵`);
  };
  ...
  const pickup = async (o) => {
    ...
    fetchOrders();
    openWA(o.phone,
      `Hello ${o.customer.split(" ")[0]} 👋, your Choma order is on its way!\n📍 Delivering to: ${o.address}\nEstimated arrival: 15–25 mins 🛵`);
      ...
      <div style={{ height:"100%",background:B.bg,display:"flex",flexDirection:"column" }}>
        ...
        <div style={{ flex:1,overflowY:"auto",padding:"16px 20px" }}>
          <div style={{ background:`linear-gradient(135deg,${B.redLight},#fff)`,
            ...
            <div style={{ fontSize:17,fontWeight:800,color:B.text,lineHeight:1.4 }}>📍 {live.address}</div>
      ...
      <div style={{ padding:"16px 20px 12px",background:B.card,
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
          ...
          <button onClick={()=>setScreen("earnings")} style={{ background:B.greenSoft,
            border:`1px solid ${B.green}30`,borderRadius:12,padding:"8px 14px",
          ...
          <>
            ...
            {mine.map(o=>(
              <div key={o.id} style={{ background:`linear-gradient(135deg,${B.orangeLight},#fff)`,
                ...
                <Btn full v="orange" onClick={e=>{ e.stopPropagation(); setActiveOrder(o); setScreen("detail"); }}>
          ...
          <Card key={o.id} style={{ marginBottom:10 }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
              <div>
                ...
                <div style={{ fontSize:12,color:B.textMid }}>{o.items.length} item{o.items.length!==1?"s":""}</div>
              </div>
...
function TrackingPage() {
  ...
  const MSGS = {
    New:"Order received — we're getting your food ready 👍",
